...
The content brief forbade changing font sizes, but it also changed the headline from `HI, I'M JACK` (12 glyphs) to `HI, I'M GNANAMOORTHI` (15). The heading is `whitespace-nowrap`, so at the specified sizes the longer name overflows and clips — roughly 28% of it disappears off-screen.
